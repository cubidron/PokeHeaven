...
export default function Mods(props: { show: boolean; close: () => void }) {
  const mods = useMods((state) => state);
        ...
        <motion.span
          ...
          <div
            ...
            <ul className="flex mt-4 gap-2 overflow-y-auto pr-2 max-h-full w-full flex-col">
              {mods.mods && mods.mods.length > 0 ? (
                mods.mods.map((mod, i) => (
                  <div
                    key={i}
                    className={`w-full flex items-center p-2 gap-4 rounded-lg ${
                      ...
                      <svg
                        ...
                        <path
                          ...
                          className={`ease-in-out duration-300 origin-center ${
                            mod.enabled
                              ? "stroke-primary scale-100"
                              : "stroke-primary mix-blend-luminosity scale-[0.85]"
                          }`}
                          strokeLinecap="round"
