...
export default function Mods({ show, close, mods }: { show: boolean; close: () => void; mods: IOptionalMod[] }) {
  const [loading, setLoading] = useState(false);
  const remote = useRemote();
  const options = useOptions();

  const handleSwitchChange = (mod: IOptionalMod) => {
    const currentServer = remote.servers?.find((s) => s.profile === options.selectedServer) || remote.servers?.[0]!;
    ...
    optionalMods = optionalMods.concat(options.optionalMods?.filter((m) => m.profile !== options.selectedServer) || []);
  ...
  return (
    <AnimatePresence mode="popLayout">
      {show && (
        <motion.span
          onClick={close}
          className="fixed inset-0 grid place-items-center backdrop-blur z-50"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
